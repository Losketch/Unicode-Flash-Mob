...
fn collect_text_chunks_impl(
    ...
    for child in parent.children() {
        ...
        let span = TextSpan {
            ...
            apply_kerning,
            features: parse_font_feature_settings(parent),
            font_optical_sizing,
...
/// Parses `font-feature-settings: normal | [ <string> <integer> ]#`.
fn parse_font_feature_settings(node: SvgNode) -> Vec<crate::FontFeature> {
    let value = node
        .ancestors()
        .find(|n| n.has_attribute(AId::FontFeatureSettings))
        .and_then(|n| n.attribute::<&str>(AId::FontFeatureSettings));
    ...
    value
        ...
        .filter_map(|part| {
            ...
            let end = part[1..].find(quote)? + 1;
            let tag = &part[1..end];
            if tag.len() != 4 || !tag.is_ascii() {
                log::warn!("Invalid font-feature-settings tag: '{tag}'");
                return None;
            }
            let tail = part[end + 1..].trim();
